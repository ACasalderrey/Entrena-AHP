...
test("el preflight acepta solo los métodos y cabeceras necesarios", () => {
  ...
  assert.equal(accepted.headers.get("access-control-allow-methods"), "GET, POST, PATCH");
  assert.equal(accepted.headers.get("access-control-allow-headers"), "content-type, x-progress-key");
  assert.equal(accepted.headers.get("cache-control"), "no-store");
  assert.equal(corsPreflightResponse(preflight(GITHUB_PAGES_ORIGIN, "PATCH")).status, 204);
